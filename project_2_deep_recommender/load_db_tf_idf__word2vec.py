...
vectorized_df = pd.DataFrame(final_post_text_df["vector"].to_list())
final_post_sadyhov_neuro = pd.concat([final_post_sadyhov, vectorized_df], axis=1)

import json

final_post_sadyhov_neuro["vector"] = final_post_sadyhov_neuro["vector"].apply(lambda x: json.dumps(x.tolist()))
